...
recommended_courses = Electrical["recommended"] # semester wise recommended plan for EE from dept.py as list of lists
    ...
    for course_code in course_list:
        if course_code == "DE":
            for de_code in Electrical["courses"]["DE"]:
                if de_code in all_courses:  #safety check
                    # Parse prerequisites for DE courses
                    prereq_string = all_courses[de_code].get("prereqs", "")
                    all_courses[de_code]["prereqs_parsed"] = parse_prereqs(prereq_string)   #parsing prereqs using function defined above
                    all_courses[de_code]["type"] = "DE"
                    selected_courses[sem_idx].append(all_courses[de_code])  # Append DE course data to semester list
...
for sem, courses in selected_courses.items(): #selected_courses is dict sem->list of course objects
    target_sem = sem if sem >= user.current_semester else user.current_semester #future semesters only
...
# Add constraints so that no two overlapping courses are in the entire degree plan
for i in range(len(overlap_list)):
    for j in range(i + 1, len(overlap_list)):
        course1 = overlap_list[i]   
        course2 = overlap_list[j]

        vars_course1 = []   #list of bool vars for course1
        vars_course2 = []   #list of bool vars for course2

        # collect all semester variables for course1 and course2
        for sem in courses_left.keys():
            if (sem, course1) in course_vars:
                vars_course1.append(course_vars[(sem, course1)])
            if (sem, course2) in course_vars:
                vars_course2.append(course_vars[(sem, course2)])

        # add global (entire degree) constraint
        if vars_course1 or vars_course2:
            model.Add(sum(vars_course1) + sum(vars_course2) <= 1)

print("Global overlap constraints added")
